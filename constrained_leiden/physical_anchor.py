...
- v3 架构中，chunk_id 的语义升级为 sent_id（句子级物理锚点）
  叶节点的 chunk_ids = {sent_id}，精确到句子级
  超节点（聚合后）继承所有子节点的 sent_id 并集
- 结构熵 H_structure 衡量一个社区内节点来源的物理分散程度
- H=0 表示社区内所有节点来自同一句子（最纯净，底层 λ 极大时的目标状态）
- H=log(k) 表示节点均匀分散于 k 个不同句子/段落/文档（最混乱）
- 随 λ 退火，H 逐渐增大，跨句子/段落/文档的语义融合逐步发生
"""
